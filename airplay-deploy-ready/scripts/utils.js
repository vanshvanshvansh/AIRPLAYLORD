...
function getAspectCorrectedCoords(normalizedPoint, videoEl, canvasEl) {
  ...
  // Where is the video ACTUALLY rendered on screen right now? In solo mode it
  // fills the whole stage; in a 2-peer call it's squeezed into one half of a
  // side-by-side (or stacked, on mobile) grid. Reading the live bounding rect
  // means this stays correct no matter how the video tile is laid out.
  const tileRect = (videoEl && videoEl.isConnected) ? videoEl.getBoundingClientRect() : canvasRect;
  const tileWidth = tileRect.width || canvasWidth;
  const tileHeight = tileRect.height || canvasHeight;

  // Tile position relative to the canvas (0,0 when solo/full-screen, offset
  // when the video sits in one half of the grid).
  const tileOffsetX = tileRect.left - canvasRect.left;
  const tileOffsetY = tileRect.top - canvasRect.top;

  // Video source aspect vs the tile's on-screen aspect (object-fit: cover crop math)
  ...
  if (tileAspect > videoAspect) {
    // Tile is wider than the video feed -> cropped vertically (top & bottom)
    renderHeight = tileWidth / videoAspect;
    cropOffsetY = (renderHeight - tileHeight) / 2;
  } else {
    // Tile is taller than the video feed -> cropped horizontally (left & right)
    renderWidth = tileHeight * videoAspect;
    cropOffsetX = (renderWidth - tileWidth) / 2;
  }

  const px = tileOffsetX + normalizedPoint.x * renderWidth - cropOffsetX;
  const py = tileOffsetY + normalizedPoint.y * renderHeight - cropOffsetY;
