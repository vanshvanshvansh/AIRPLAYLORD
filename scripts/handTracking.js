...
class HandTracker {
  ...
  drawSkeleton() {
    ...
    // Draw Joint Nodes
    // PERF: this ran ctx.shadowBlur on all 21 hand landmarks EVERY FRAME —
    // the single biggest steady-state cost in the app, since it fired
    // continuously any time a hand was visible (i.e. during basically all
    // gameplay), not just occasionally. Glow now only drawn on the 5
    // fingertips (via cached drawGlow sprite, see utils.js) and the plain
    // 16 knuckle/joint dots are flat fills with no glow at all — visually
    // still reads as a hand skeleton, at a fraction of the render cost.
    this.lastLandmarks.forEach((lm, idx) => {
      ...
      const color = idx === 8 ? '#ff0844' : '#00f2fe';

      if (isFingertip) {
        drawGlow(ctx, pt.x, pt.y, 6, color, 1.8);
      }
