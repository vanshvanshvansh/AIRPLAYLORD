...
class WebRTCManager {
  ...
  setVideoFilter(filterName) {
    ...
    if (this.localVideoElement) {
      this.localVideoElement.className = `filter-${filterName}`;
    }
  }

  // Applies a filter to the REMOTE video element on this device — used when
  // the peer picks a filter for themselves; their choice is synced over
  // SyncEngine and applied here so it actually shows up on our screen too,
  // instead of only ever being visible on the sender's own device.
  setRemoteVideoFilter(filterName) {
    if (this.remoteVideoElement) {
      this.remoteVideoElement.className = `filter-${filterName}`;
    }
  }
