...
class GameOverlayManager {
  ...
  showGameResultModal(gameId, scores, winner, playedSeconds, tdCounts = null) {
    ...
    if (gameId === 'truth' && tdCounts) {
      ...
      // BUGFIX: see .game-result-value.is-text in main.css — this text
      // breakdown was inheriting the big-monospace-nowrap style meant for
      // a short numeric score, which squashed/overlapped the words.
      if (valAEl) { valAEl.textContent = `${a.truth} Truth · ${a.dare} Dare`; valAEl.classList.add('is-text'); }
      if (valBEl) { valBEl.textContent = `${b.truth} Truth · ${b.dare} Dare`; valBEl.classList.add('is-text'); }
    ...
    if (valAEl) { valAEl.textContent = scores.peerA || 0; valAEl.classList.remove('is-text'); }
    if (valBEl) { valBEl.textContent = scores.peerB || 0; valBEl.classList.remove('is-text'); }
