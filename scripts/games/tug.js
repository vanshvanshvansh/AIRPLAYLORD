...
class TugGame {
  ...
  render(ctx, width, height, localFingertip, peerFingertip) {
    ...
    // Sliding Tug Marker
    // PERF: shadowBlur replaced with cached glow sprite (drawGlow) — see
    // utils.js. This marker redraws every frame for the whole round.
    const mx = (cx - barWidth / 2) + this.markerPos * barWidth;
    drawGlow(ctx, mx, cy, 32, '#00f2fe', 1.6);
    ctx.beginPath();
