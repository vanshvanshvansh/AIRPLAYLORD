...
class TugGame {
  ...
  start(canvas, syncEngine, timerSetting = 90, extraSettings = null) {
    ...
    if (extraSettings) {
      ...
      if (extraSettings.tugHold) this.pointLifetimeMs = (parseInt(extraSettings.tugHold) || 5) * 1000;
      if (extraSettings.sensP1) this.sensitivityP1 = parseFloat(extraSettings.sensP1) || 1.0;
      if (extraSettings.sensP2) this.sensitivityP2 = parseFloat(extraSettings.sensP2) || 1.0;
    }
  ...
  applyPullForce(role, force) {
    const sens = role === 'peerA' ? this.sensitivityP1 : this.sensitivityP2;
    const direction = role === 'peerA' ? -1 : 1;
    this.markerPos = Math.max(0, Math.min(1, this.markerPos + direction * force * 0.04 * sens));
  ...
  updatePressurePoints(now, width, height, myRole, localFingertip) {
    ...
    if (localFingertip) {
      ...
      const sens = myRole === 'peerA' ? this.sensitivityP1 : this.sensitivityP2;
      const radius = this.POINT_RADIUS * sens;
      const stillActive = [];
      let hits = 0;
  ...
  drawPressurePoints(ctx, myRole, now) {
    ...
    const sens = myRole === 'peerA' ? this.sensitivityP1 : this.sensitivityP2;
    const rad = this.POINT_RADIUS * sens;
