...
class TruthGame {
  constructor(overlayManager) {
    ...
    // Whether the active Truth/Dare prompt card is collapsed down to a
    // small floating ball (see renderMinimizedBall) — lets a player peek
    // at the prompt, then get it out of the way of the video call without
    // losing it, then bring it back up any time before the next spin.
  ...
  render(ctx, width, height, localFingertip, peerFingertip) {
    ...
    if (!this.isSpinning && this.choiceState === 'IDLE') {
      ...
      if (this.minimized) {
        this.renderMinimizedBall(ctx, width, height, localFingertip);
      } else {
        this.renderPromptCard(ctx, width, height, localFingertip);
      }
    }
  }
  ...
  // BUGFIX (prompt card blocking the video call / covering a face on
  // phones): this used to render centered on the bottle (screen middle),
  // which on a portrait phone layout sits right on top of the remote
  // player's face. Now anchored near the BOTTOM of the screen, clear of
  // both video tiles, with a minimize button so it can be tucked away
  // (see renderMinimizedBall) without losing the prompt.
  renderPromptCard(ctx, width, height, localFingertip) {
    ctx.save();
    const cx = width / 2;
    const boxWidth = Math.min(440, width - 32);
    const boxHeight = 138;
    // Sits just above the (auto-hiding) bottom control bar instead of
    // dead-center over the video tiles.
    const boxTop = height - boxHeight - 96;

    ctx.beginPath();
    ctx.roundRect(cx - boxWidth / 2, boxTop, boxWidth, boxHeight, 20);
    ctx.fillStyle = 'rgba(18, 24, 38, 0.97)';
    ctx.strokeStyle = this.chosenType === 'TRUTH' ? '#00f2fe' : '#ff0844';
    ctx.lineWidth = 3;
    ctx.fill();
    ctx.stroke();

    // Minimize button (top-right corner of the card)
    const minX = cx + boxWidth / 2 - 26;
    const minY = boxTop + 24;
    ctx.beginPath();
    ctx.arc(minX, minY, 15, 0, 2 * Math.PI);
    ...
    ctx.font = `700 ${scaleFont(ctx.canvas, 16)}px Outfit, sans-serif`;
    ctx.textAlign = 'center';
    ctx.fillText('—', minX, minY + 5);

    ctx.font = `700 ${scaleFont(ctx.canvas, 18)}px Outfit, sans-serif`;
    ctx.fillStyle = this.chosenType === 'TRUTH' ? '#00f2fe' : '#ff0844';
    ctx.fillText(`${this.chosenType}:`, cx - 14, boxTop + 30);

    ctx.font = `500 ${scaleFont(ctx.canvas, 15)}px Outfit, sans-serif`;
    ctx.fillStyle = '#ffffff';
    wrapCanvasText(ctx, this.currentPrompt || '', cx, boxTop + 62, boxWidth - 40, scaleFont(ctx.canvas, 20));

    // Next Turn / Spin Again Hover Button
    const bx = cx;
    const by = boxTop + boxHeight - 20;
    ctx.beginPath();
    ...
    ctx.font = `700 ${scaleFont(ctx.canvas, 13)}px Outfit, sans-serif`;
    ctx.fillStyle = '#00f2fe';
    ctx.fillText('🔄 SPIN AGAIN', bx, by + 4);

    if (localFingertip) {
      const fx = localFingertip.px;
      const fy = localFingertip.py;

      if (Math.sqrt((fx - minX) ** 2 + (fy - minY) ** 2) < 20) {
        this.minimized = true;
      } else if (Math.abs(fx - bx) < 90 && Math.abs(fy - by) < 20) {
        this.resetTurnState();
      }
    }
  ...
  // Collapsed state: a small floating tap-to-expand ball tucked in a
  // bottom corner, well clear of both video tiles — so a player can peek
  // at the prompt, put it away, keep talking/playing, then bring it back
  // with a tap any time before the next spin. Position doesn't overlap
  // the auto-hiding control bar since it sits slightly above/beside it.
  renderMinimizedBall(ctx, width, height, localFingertip) {
    ctx.save();
    const r = 26;
    const bx = width - r - 18;
    const by = height - r - 100;
    const color = this.chosenType === 'TRUTH' ? '#00f2fe' : '#ff0844';

    drawGlow(ctx, bx, by, r, color, 1.6);
    ctx.beginPath();
    ...
    ctx.font = `700 ${scaleFont(ctx.canvas, 12)}px Outfit, sans-serif`;
    ctx.textAlign = 'center';
    ctx.fillStyle = color;
    ctx.fillText(this.chosenType === 'TRUTH' ? 'T' : 'D', bx, by + 4);

    if (localFingertip) {
      const fx = localFingertip.px;
      const fy = localFingertip.py;
      if (Math.sqrt((fx - bx) ** 2 + (fy - by) ** 2) < r + 6) {
        this.minimized = false;
      }
    }
