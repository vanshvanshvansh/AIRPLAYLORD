...
class TruthGame {
  ...
  renderChoiceButtons(ctx, width, height, localFingertip) {
    ...
    ctx.textAlign = 'center';
    ctx.fillStyle = '#ffffff';
    ctx.fillText(`${this.selectedPlayer === 'peerA' ? 'P1 (Pink)' : 'P2 (Cyan)'} — Choose Truth or Dare!`, cx, cy - 50);
  ...
  renderPromptCard(ctx, cx, cy, localFingertip) {
    ctx.save();
    ctx.beginPath();
    ctx.roundRect(cx - 260, cy + 60, 520, 160, 20);
    ...
    ctx.fillStyle = '#ffffff';
    ctx.fillText(this.currentPrompt || '', cx, cy + 135);
