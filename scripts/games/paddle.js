...
class PaddleGame {
  ...
  start(canvas, syncEngine, timerSetting = 90, extraSettings = null) {
    ...
    // Apply Hand Sensitivity setting to paddle widths — one shared value
    // for both players (fair, adjustable 0.5x-4.0x rather than fixed
    // Low/Normal/High presets).
    let sens = (extraSettings && extraSettings.sensitivity) ? parseFloat(extraSettings.sensitivity) : 1.0;
    if (!Number.isFinite(sens)) sens = 1.0;
    sens = Math.max(0.5, Math.min(4.0, sens));

    this.paddleA.width = 0.2 * sens;
    this.paddleB.width = 0.2 * sens;
  ...
  render(ctx, width, height, localFingertip, peerFingertip) {
    ...
    // Render Soft Border Goal Glow — `goalFlash.role` is always the
    // SCORER (set in updatePhysics() right next to `overlay.updateScore`),
    // and paddleA (Player 1/Red) sits at the bottom while paddleB
    // (Player 2/Blue) sits at the top. So this glow is deliberately drawn
    // on the scorer's own paddle side, not the side the ball crossed
    // (i.e. not the side that conceded) — e.g. when Blue's paddle at the
    // top misses and Red scores, the glow shows at the BOTTOM (Red's
    // side), not the top where the ball actually went out.
    if (this.goalFlash) {
      ...
      if (elapsed < 500) {
        ...
        if (this.goalFlash.role === 'peerA') {
          // Bottom border glow — Player 1 (Red) scored, and Red's own
          // paddle/side is the bottom.
          ctx.fillRect(0, height - 24, width, 24);
        } else {
          // Top border glow — Player 2 (Blue) scored, and Blue's own
          // paddle/side is the top.
