...
class SyncEngine {
  constructor(roomId, isHost = false, userName = "Player") {
    ...
    // Fingertip position (`inputs/peerA` / `inputs/peerB`) is written up to
    // once per animation frame while hand-tracking is active — on mobile,
    // routing that through Firebase every frame was a steady source of
    // jank (each write is a network round trip). Matched by prefix (not
    // added to REALTIME_PATHS, which is exact-match) since the path
    // includes the dynamic peer role.
    this.REALTIME_PREFIXES = ['inputs/'];
  ...
  write(path, data) {
    const isRealtimePath = this.REALTIME_PATHS.has(path) || this.REALTIME_PREFIXES.some((p) => path.startsWith(p));
    if (isRealtimePath && this.dataChannel && this.dataChannel.readyState === 'open') {
      try {
