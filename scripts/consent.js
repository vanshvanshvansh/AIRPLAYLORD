// AirPlay — Join Request & Game Invite Consent Manager (v2 Updated with Extra Settings)
...
class ConsentManager {
  ...
  setupListeners() {
    ...
    this.sync.listen('game/invite', (invite) => {
      if (invite && invite.status === 'pending' && invite.from !== this.sync.peerRole) {
        this.showGameInviteModal(invite.game, invite.fromName || 'Peer', invite.timerDuration, invite.extraSettings);
      }
    });
  }
  ...
  showGameInviteModal(gameId, fromName, timerDuration = 45, extraSettings = null) {
    const titleMap = {
    ...
    acceptBtn.onclick = debounce(() => {
      ...
      if (this.onGameInviteReceived) {
        this.onGameInviteReceived(gameId, true, timerDuration, extraSettings);
      }
    });
    ...
    declineBtn.onclick = debounce(() => {
      ...
      if (this.onGameInviteReceived) {
        this.onGameInviteReceived(gameId, false, timerDuration, extraSettings);
      }
    });
  }
  ...
  sendGameInvite(gameId, timerDuration = 45, extraSettings = null) {
